...
class Message extends Component {
    ...
    componentDidMount() {
        ...
        database().ref(`/status/${pushKey}/allComments`).on('value', snap => {
            let data = snap.val();
            let messages = [];
            if (data) {
    ...
    _onPress = () => {
        if (this.state.message.trim()) {
            ...
            let pushKey = this.props.navigation.state.params.pushKey;
            database().ref(`/status/${pushKey}/allComments`).push({ name: this.props.user.userName, comment: this.state.message, time: Date.now(), photo: this.state.photo })
                .then(() => {
                    database().ref(`/status/${pushKey}`).update({ comments: comment + 1 })
                })
            Keyboard.dismiss();
                                ...
                                <View style={{ width: "20%", justifyContent: "center", alignItems: "center" }}>
                                    {
                                        msg.photo ? <Thumbnail style={{ width: 40, height: 40, borderRadius: 20, borderWidth: 2.0, borderColor: Styles.theme.borderColor }} source={{ uri: msg.photo }} /> :
                                            <Thumbnail style={{ width: 40, height: 40, borderRadius: 20, borderWidth: 2.0, borderColor: Styles.theme.borderColor }} source={{ uri: 'https://res.cloudinary.com/atif786/image/upload/v1572551774/dokz6y90fmngmiland17.jpg' }} />
                                    }
                                </View>
